...
import { AccordionIcon, AccordionToggleIcon, TableWrapper, ViewMoreButton, ViewMoreWrapper,TableContentWrapper} from './StyledMipMskuTable';
import { Table, TableBody, TableCell, TableRow} from '@material-ui/core';
import { IconAngleDown } from '@hs/icons';

export const MipMskuTable: React.FC<MipMskuTableProps> = ({ tableData,accordian=false, accordianCount=0 }: MipMskuTableProps) => {
    ...
    function toggleAccordian($event){
        ...
        let accordianBtnTextState = isAccordianOpen ? `${accordianCount} More` : 'Less';
    ...
    useEffect(()=>{
        setAccordianBtnText(accordian && accordianCount > 0 ? !isAccordianOpen ? `${accordianCount} More` : `Less` : '')
    },[accordian])
                        ...
                        return (
                            <TableRow key={`table${index}`}>
                                {item.map((colItem, colIndex) => {
                                    return (<TableCell key={`tableCell${colIndex}`} variant={index == 0 ? "head" : "body"}>{colItem}</TableCell>)
                                })
                                }
                            </TableRow>
                        )
