...
export const ProductNamePrice: FC<IProductNamePriceProps> = ({
  ...
  if (retailPrice) {
    const getDiscountDetails = () => {
      return selectedSku && !retailPriceMax && discount && discount > 2 && regularPrice && regularPrice > retailPrice
        ? true
        : false;
    };

    const getRetailPrice = () => {
      return retailPriceMax && retailPriceMax > 0 && !selectedSku && retailPrice != retailPriceMax ? true : false;
    };
      ...
      <>
        ...
        <ProductPricingWrapper>
          <ProductNamePriceWrapper>
            ...
            {getRetailPrice() && <ProductPrice>&nbsp;-&nbsp;₹{getFormattedPrice(retailPriceMax)}</ProductPrice>}
            {getDiscountDetails() && (
              <ProductOfferPrice>
                <ProductVendorPrice>₹{getFormattedPrice(regularPrice)}</ProductVendorPrice>
